...
bool ImGui::CheckboxEX(const char* label, bool* v, const char* label2, int ID)
{
    int timer = 0;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    bool gds = true;
    static float animation_r[10000];
    static float animation_g[10000];
    static float animation_b[10000];
    ...
    if (gds)
    {
        ...
        {
            animation_timer[ID]++;
            animation_r[ID] = animation_r[ID] + 0.02f * g_Options.menu_color[0];
            if (animation_r[ID] > g_Options.menu_color[0])
            {
                animation_r[ID] = g_Options.menu_color[0];
            }
            animation_g[ID] = animation_g[ID] + 0.02f * g_Options.menu_color[1];
            if (animation_g[ID] > g_Options.menu_color[1])
            {
                animation_g[ID] = g_Options.menu_color[1];
            }
            animation_b[ID] = animation_b[ID] +0.02f * g_Options.menu_color[2];
            if (animation_b[ID] > g_Options.menu_color[2])
            {
                animation_b[ID] = g_Options.menu_color[2];
            }
            if (animation_timer[ID] > 300)
            {
                animation_timer[ID] = 300;
            }

        }
        else
        {
            animation_timer[ID] = 1;
            animation_r[ID] = animation_r[ID] - 0.02f * g_Options.menu_color[0];
            if (animation_r[ID] < 0.14f)
            {
                animation_r[ID] = 0.14f;
            }
            animation_g[ID] = animation_g[ID] - 0.02f * g_Options.menu_color[1];
            if (animation_g[ID] < 0.13f)
            {
                animation_g[ID] = 0.13f;
            }
            animation_b[ID] = animation_b[ID] - 0.02f * g_Options.menu_color[2];
            if (animation_b[ID] < 0.14f)
            {
                animation_b[ID] = 0.14f;
            }
    ...
    //animation[id2]
    window->DrawList->AddRect(check_bb.Min + ImVec2(3, 3), check_bb.Max - ImVec2(3, 3), ImColor(animation_r[ID], animation_g[ID], animation_b[ID], 1.f), style.FrameRounding - 1);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect total_bb(frame_bb.Min + ImVec2(0,3), frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 1.3f));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Min.x + style.ItemInnerSpacing.x + (283 / 2) - label_size.x / 2, frame_bb.Min.y + style.FramePadding.y - 14), label);
