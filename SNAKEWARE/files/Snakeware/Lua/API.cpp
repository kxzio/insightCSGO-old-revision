...
namespace lua {
	...
	namespace ns_entity {
		...
		Vector GetCollideable_MAX(C_BasePlayer* entity) {
			return entity->m_hActiveWeapon()->GetCollideable()->OBBMaxs();
		}
		Vector GetCollideable_MIN(C_BasePlayer* entity) {
			return entity->m_hActiveWeapon()->GetCollideable()->OBBMins();
		}
	...
	void init_command() {
		...
		lua_state.new_usertype<Vector2D>("Vector2",
			"x", &Vector2D::x,
			"y", &Vector2D::y
			);

		lua_state.new_usertype<Vector>("Vector3",
			"x", &Vector::x,
			"y", &Vector::y,
			"z", &Vector::z
			);
