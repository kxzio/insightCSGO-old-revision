...
void RenderRageBotTab()
{
	...
	case 0:
	{
		...
		ImGui::BeginChild("Ragebot", ImVec2(310, 360), true);
		{
			...
			ImGui::CheckboxEX("Enabled##rage", &g_Options.ragebot_enabled, "Turn on ragebot",1);
			...
			ImGui::CheckboxEX("Silent", &g_Options.ragebot_silent, "NoVisible aim", 2);
			
			ImGui::CheckboxEX("Remove recoil", &g_Options.ragebot_remove_recoil, "No recoil with rifles", 3);
			...
			ImGui::Text("AutoPeek");
			ImGuiEX::Hotkey("##autopeek	  ", &g_Options.autopeek_key, ImVec2(150, 20));
			//##3side	  
			const char* autostop[] = { "Default","Maximum","Forced low","Full" };
			const char* autostop_if[] = { "If low hitchance","Always" };

			ImGui::CheckboxEX("Auto-scope", &g_Options.ragebot_autoscope, "autoscope with snipers", 4);
			ImGui::CheckboxEX("Auto-stop", &g_Options.ragebot_autostop, "automatic stop", 5);
			ImGui::CheckboxEX("Between shots", &g_Options.ragebot_autostop_bs, "stop you after shots", 6);
		...
		ImGui::BeginChild("Settings", ImVec2(310, 210), true);
		{
			...
			ImGui::CheckboxEX("Auto-fire", &g_Options.ragebot_autofire, "Shoot automatic", 7);
			...
			ImGui::CheckboxEX("Auto-crouch", &g_Options.ragebot_autocrouch[curGroup], "Crouch when shooting", 8);
...
void RenderEspTab()
{
	...
	case 1:
	{
		...
		ImGui::BeginChild("Chams/Glow", ImVec2(310, 360), true);
		{
				...
				ImGui::CheckboxEX("Fake-chams", &g_Options.chams_fake, "Draw desync", 53);
	...
	case 0:
	{
		...
		ImGui::BeginChild("Config", ImVec2(310, 360), true);
		{
			...
			if (!current_config.empty()) {
				...
				draw_lua_items();	
				// так а теперь надо как то придумать что бы выводить в меню это говнецо из луа
